...
describe("<AlertStripeContainer/>", () => {
  test("Bruker uten oppf.perioder og ikke under oppf. viser en advarsel - veileder.", () => {
    ...
    jest
      .spyOn(AppContext, "useUserInfoContext")
      .mockImplementation(() => userInfo);
    jest
      .spyOn(AppContext, "useOppfolgingContext")
      .mockImplementation(() => oppfolgingData);
    const wrapper = shallow(<AlertStripeContainer />);
    expect(
      wrapper.find("[data-ikke-reg-veileder-test]").props().visible
    ).toBeTruthy();
  });
  test("Bruker uten oppf.perioder og ikke under oppf. viser en advarsel - bruker. ", () => {
    ...
    const wrapper = shallow(<AlertStripeContainer />);
    expect(
      wrapper.find("[data-ikke-reg-bruker-test]").props().visible
    ).toBeTruthy();
  });
  test("Bruker med oppf.perioder og ikke under oppf. viser en advarsel - bruker. ", () => {
    ...
    const wrapper = shallow(<AlertStripeContainer />);
    expect(
      wrapper.find("[data-har-oppfP-bruker-test]").props().visible
    ).toBeTruthy();
  });
...
describe("<Dialog/>", () => {
  test("Bruker ikke under oppf. skjuler dialogcontroller og viser fortsatt henvendelser", () => {
    ...
    jest
      ...
      .mockImplementation(() => oppfolgingData);
    const wrapper = mount(
      <Dialog dialogData={dialoger} {...lagRouterProps("1")} />
    );
...
function lagRouterProps(
  dialogId: string
): RouteComponentProps<{ dialogId?: string }> {
  return {
